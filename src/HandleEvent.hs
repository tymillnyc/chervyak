...
handleText :: Event -> AppState -> IO AppState
handleText (EventKey (SpecialKey keyButton) Down _ _) state = 
  case keyButton of
    KeyEnter -> if ((length(name state)) /= 0) then do
      newId <- createAUser (name state) (connection state)
      return (state {screen = Menu, snakeId = newId}) else return state
    _ -> return state
...
handleMotion :: Event -> AppState -> IO AppState
handleMotion (EventKey (SpecialKey keyButton) Down _ _) state = 
  case keyButton of
  KeyLeft -> return (changeDirection (direction state) CrawlLeft state)
  KeyRight -> return (changeDirection (direction state) CrawlRight state)
  KeyUp -> return (changeDirection (direction state) CrawlUp state)
  KeyDown -> return (changeDirection (direction state) CrawlDown state)
  KeySpace -> if isGameOver state 
     then do
      writeResultToTable ((length(snakeCoordinates state) - 2) * 5) (snakeId state) (connection state)
      top10 <- getTop10 (connection state)
      return ((initState (connection state) top10) {screen = Menu, snakeId = (snakeId state), name = (name state)})
     else return state
  _ -> return state
handleMotion _ state = return state
